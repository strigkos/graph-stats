				...
				<?php endif; ?>
					...
					<div class="entry-content" style="margin-left:10px; margin-right:10px">
						...
						<?php
							...
							if  ( isset($_GET['reg_ids']) )
							{
								$sanitize_text_field = sanitize_text_field($_GET['reg_ids']);
								$reg_ids = explode(',', $sanitize_text_field);
							}
							else
							{
								$reg_ids = [];
						...
						<tfoot>
							<tr>
								...
								<?php
									...
									if ( in_array($current_user->user_login, ['eeep', 'mike', 'uat', 'wpadmin'])) { echo '<th>' . '' . '</th>'; }
								?>
							</tr>
						</tfoot>
						...
						<?php
							...
							while ( $my_pods->fetch() )
							{
								if ( in_array($my_pods->id(), $reg_ids) || empty($reg_ids) ) 
								{
									/// Hops
					...
					<script>
						...
						$(document).ready(function () {
							...
							// Function to normalize Greek text
							...
							/// let all_adata = table.rows().data().toArray();
							$.fn.dataTable.ext.search.push(function (settings, data, data_index)
							{
							...
							table.on('draw', function () {
								...
								if ( type == 'line' || type == 'spline' ||  type == 'column' )
								{
									...
									$('#second_table tbody tr').each(function() {
										let ana = 0;
										let time = $(this).find('td').eq(ana).text();  // Get the time value (first column)
