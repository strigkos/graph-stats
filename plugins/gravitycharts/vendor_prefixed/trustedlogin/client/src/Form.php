...
final class Form {
	...
	public function get_auth_header_html() {
		...
		$content = array(
			...
			'revoke_access_button' => sprintf( '<a href="%1$s" class="button button-danger alignright tl-client-revoke-button">%2$s</a>', $revoke_url, esc_html__( 'Revoke Access', 'gk-gravitycharts' ) ),
			// translators: %s is the display name of the user who granted access.
			'auth_meta'            => sprintf( esc_html__( 'Created %1$s ago by %2$s', 'gk-gravitycharts' ), human_time_diff( strtotime( $support_user->user_registered ) ), $auth_meta ),
		);
	...
	private function get_debug_data_consent_html() {
		...
		$output = sprintf(
			...
			strtr(
				esc_html__( 'Include the [link]Site Health[/link] troubleshooting report', 'gk-gravitycharts' ),
				array(
	...
	public function get_button( $atts = array() ) {
		...
		$defaults = array(
			...
			// translators: %s is replaced with the name of the software developer (e.g. "Acme Widgets").
			'exists_text' => sprintf( esc_html__( 'Extend %s Access', 'gk-gravitycharts' ), $this->config->get_display_name(), ucwords( human_time_diff( time(), time() + (int) $this->config->get_setting( 'decay' ) ) ) ),
			'size'        => 'hero',
		...
		$translations = array(
			...
			'status'  => array(
				...
				'extending'          => array(
					...
					'content' => sprintf( __( 'Extending support access for %1$s by %2$s', 'gk-gravitycharts' ), $vendor_title, human_time_diff( time(), time() + (int) $this->config->get_setting( 'decay' ) ) ),
				),
				'syncing'            => array(
					// translators: %1$s is the vendor title.
	...
	public function output_support_users( $print_and_return = true ) {
		...
		if ( empty( $support_users ) ) {
			...
			// translators: %s is replaced with the name of the software developer (e.g. "Acme Widgets").
			$return = '<h3>' . sprintf( esc_html__( 'No %s users exist.', 'gk-gravitycharts' ), esc_html( $this->config->get_setting( 'vendor/title' ) ) ) . '</h3>';
...
EOD;
			$access_key_output   = sprintf(
				...
				sprintf( esc_html__( 'The access key is not a password; only %1$s will be able to access your site using this code. You may share this access key on support forums.', 'gk-gravitycharts' ), esc_html( $this->support_user->get_first()->display_name ) )
			);
		}
